...
{
    int num;
    for(num = 2;num != 254;num++){
        //char pcnet[255] = "10.1.11.";   // 网络号
        char pcnet[255] = "192.168.226.";   // 网络号
        char pcnum[20] = "";            // 主机号
        // itoa(i,num,10);用于把一个整数转换为字符串，i 需要转换成字符串的数字，num 转换后保存字符串的变量，10 转换数字的基数（即进制）
        itoa(num,pcnum,10);
        strcat(pcnet,pcnum);   // 将网络号和主机号拼接起来
        ...
        //得到IP地址
        u_long ulDestIP = inet_addr( pcnet );
        ...
        cout<< "-------------------------------------->>     " << pcnet;
        if(!bReachDestHost)     // 说明达不到目的地
        {
            cout<<"     离线"<<'\t'<< " " <<"Request timed out."<<endl;
        }
        else
        {
            cout<<"     在线"<<endl;
